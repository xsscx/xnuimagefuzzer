...
function updateStats() {
  document.getElementById('stats').textContent =
    `${IMAGES.length} images | OK ${totalLoaded} loaded | FAIL ${totalErrors} errors | SLOW ${totalSlow} (>2s)`;
}
...
def serialize_images_for_inline_script(image_list):
    """Return JSON safe to embed inside an inline <script> block."""
    return json.dumps(image_list).replace('</', '<\\/')


def resolve_image_path(image_dir, request_path):
    """Resolve a gallery request path to a file within image_dir."""
    image_root = os.path.abspath(image_dir)
    filename = request_path[8:]  # strip /images/
    filepath = os.path.abspath(os.path.join(image_root, filename))
    try:
        if os.path.commonpath((image_root, filepath)) != image_root:
...
class FuzzGalleryHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/' or path == '/index.html':
            ...
            self.end_headers()
            inline_images = serialize_images_for_inline_script(self.image_list)
            html = GALLERY_HTML.replace('__IMAGES_JSON__', inline_images)
            self.wfile.write(html.encode('utf-8'))
            return

        if path.startswith('/images/'):
            filepath, filename = resolve_image_path(self.image_dir, path)
